...
- em um teste recente, acabei me deparando com uma algo relativamente raro nas minhas análises de aplicações web. ao analisar as requests no burp suite, todo o tráfego com a api da aplicação era criptografado! (alguns request headers foram omitidos)
    ![crypto](image.png)
- com isso, voltei ao browser e dei uma olhada no wappalyzer pra checar quais tecnologias eram utilizadas pela aplicação.
    ![wappalyzer](image-1.png)
- como podemos ver, o framework react.js é utilizado no front-end. sabendo disso, abri o devtools pra começar um code review, visto que por padrão o react armazena toda a lógica do front-end da aplicação em client-side, ou seja, provavelmente a lógica da criptografia implementada estaria ali.
...
- e a função `decryptData()`, realiza a descriptografia
    ![decrypt](image-4.png)
- sabendo disso, podemos pesquisar por referências à essas duas funções no contexto do tratamento de requests e responses, para assim podermos interceptar os dados que elas recebem e poder interagir com a api em plaintext.
...
- uma request que me chamou a atenção no fluxo de reload da dashboard foi uma para `/api/cash_value/[id]`, que retornava quanto dinheiro eu havia injetado na plataforma, principalmente após a captura da response de `/api/profile`, que revelou que o id do meu usuario era exatamente aquele mesmo id.
    ![bola](image-9.png)
- modificando a url da request para `/api/cash_value/32`, confirmamos a existencia de um idor/bola
    ![alt text](image-10.png)
- sabendo disso, decidi criar um script para percorrer vários id's e retornar quanto cada usuário tinha colocado na plataforma.
    ```js
